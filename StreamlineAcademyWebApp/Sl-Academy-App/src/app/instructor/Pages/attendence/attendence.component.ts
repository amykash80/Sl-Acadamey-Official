...
export class AttendenceComponent {
  ...
  saveAttendance() {
    ...
    this.filteredStudentList.forEach((student) => {
      this.attendanceStatusList.push(
        this.attendanceMap[student.id!] ?? AttendenceStatus.Absent
      );
    ...
    if (attendanceData.AttendenceStatus.length < 1) {
      this.sharedService.showErrorToast('please mark attendence first');
    }
    else{
      this.instructorService.saveAttendance(attendanceData).subscribe(
        (response) => {
          if (response.isSuccess) {
            this.sharedService.showSuccessToast('Attendance marked successfully');
        ...
        (error) => {
          ...
          this.sharedService.showErrorToast('Error saving attendance');
        }
      );
    }
